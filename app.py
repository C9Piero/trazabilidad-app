...
def generar_pdf_constancia(
    ...
    """Genera la constancia idéntica de transformación de uniformes en desuso."""
    buffer = io.BytesIO()
    ...
    # Estructura de la tabla oficial
    data_tabla = [
        [
    ...
    # Fecha final
    fecha_cierre_texto = (
        f"Lima, {fe_fin_dt.day} de {mes_str} de {fe_fin_dt.year}"
    )
...
if not st.session_state.autenticado:
    ...
    with st.sidebar:
        ...
        if st.button(
            ...
            type=(
                "primary"
                if st.session_state.pestaña_activa
                == "⚡     Carga Rápida Histórica"
                else "secondary"
            ),
        ):[cite: 1]
    ...
    if st.session_state.pestaña_activa == "⚡     Carga Rápida Histórica":
        ...
        st.caption(
            "Utiliza este formulario simplificado para ingresar rápidamente"
            " métricas consolidadas de proyectos pasados directamente al"
            " historial y métricas generales."
        )[cite: 1]
        ...
        with st.container(border=True):
            ...
            fast_f_ini = rq5.date_input(
                "Fecha Inicio",
                value=datetime.date.today(),
                format="DD/MM/YYYY",
            )[cite: 1]
            fast_f_fin = rq6.date_input(
                "Fecha Término",
                value=datetime.date.today(),
                format="DD/MM/YYYY",
            )[cite: 1]
            ...
            cli_clean = (
                fast_cliente.strip() if fast_cliente.strip() else "EMPRESA"
            )[cite: 1]
        ...
        with st.container(border=True):
            ...
            fast_peso = rm1.number_input(
                "Material Procesado Total (kg) *", min_value=0.0, step=0.1
            )[cite: 1]
            fast_unid = rm2.number_input(
                "Unidades Producidas *", min_value=0, step=1
            )[cite: 1]
            fast_co2 = rm3.number_input(
                "CO₂e Neto Evitado (kg) *", min_value=0.0, step=0.1
            )[cite: 1]

            rm4, rm5, rm6 = st.columns(3)[cite: 1]
            fast_horas = rm4.number_input(
                "Horas de Trabajo Generadas *", min_value=0.0, step=0.5
            )[cite: 1]
            fast_personas = rm5.number_input(
                "Personas / Beneficiarios *", min_value=0, step=1
            )[cite: 1]
            fast_aprovechamiento = rm6.number_input(
                "% Aprovechamiento *",
                min_value=0.0,
                max_value=100.0,
                value=100.0,
        ...
        if st.button(
            "🚀 Guardar Proyecto Histórico Directamente",
            type="primary",
            use_container_width=True,
        ):[cite: 1]
            if not fast_cliente.strip():
                ...
                try:
                    ...
                    st.success(
                        f"✅ ¡Proyecto **{fast_cliente}** registrado"
                        " exitosamente en el Histórico!"
                    )[cite: 1]
                    st.toast("⚡ Guardado rápido completado")[cite: 1]
                except Exception as e:
    ...
    # --- VISTA: PROYECTOS EN PROCESO ---
        ...
        st.caption(
            "Proyectos guardados pendientes de culminación o emisión"
            " definitiva."
        )[cite: 1]

        proyectos_lista = cargar_proyectos()[cite: 1]
        borradores = [
            p for p in proyectos_lista if p.get("estado") == "EN_PROCESO"
        ][cite: 1]
        ...
        if borradores:
            for b in borradores:
                with st.container(border=True):
                    ...
                    bc2.markdown(
                        f"**Tipo:** {b.get('tipo_proyecto', 'Upcycling')}"
                    )[cite: 1]
    ...
    # --- VISTA: DASHBOARD 2026 ---
        ...
        st.caption(
            "Métricas consolidadas de todos los proyectos completados en el"
            " sistema."
        )[cite: 1]

        proyectos_lista = cargar_proyectos()[cite: 1]
        completados = [
            p for p in proyectos_lista if p.get("estado") == "COMPLETADO"
        ][cite: 1]

        tot_peso = sum(
            [float(p.get("peso_recibido", 0) or 0) for p in completados]
        )[cite: 1]
        tot_co2 = sum([float(p.get("co2_neto", 0) or 0) for p in completados])[cite: 1]
        tot_horas = sum(
            [float(p.get("horas_totales", 0) or 0) for p in completados]
        )[cite: 1]
        tot_unids = sum(
            [int(p.get("productos_unids", 0) or 0) for p in completados]
        )[cite: 1]
        ...
        st.write("")[cite: 1]
        st.markdown("##### 📋 Resumen General de Proyectos Registrados")[cite: 1]
        if completados:
            ...
            df_tabla = df_comp[
                [c for c in columnas_mostrar if c in df_comp.columns]
            ].rename(columns=nombres_cols)[cite: 1]
            st.dataframe(df_tabla, use_container_width=True, hide_index=True)[cite: 1]
        else:
            st.info(
                "📭 Aún no hay proyectos completados para mostrar en las"
                " métricas."
            )[cite: 1]
    ...
    # --- VISTA: HISTORIAL COMPLETO ---
        ...
        if proyectos_lista:
            for p in proyectos_lista:
                with st.container(border=True):
                    hc1, hc2, hc3, hc4, hc5 = st.columns([2.8, 1.8, 1.8, 2.0, 1.0])[cite: 1]
                    ...
                    hc3.markdown(
                        f"Peso: `{float(p.get('peso_recibido', 0) or 0):.2f} kg`"
                    )[cite: 1]
                    ...
                    if pdf_link:
                        ...
                        hc4.caption("📄 Sin PDF en nube")[cite: 1]
    ...
    # --- VISTA: NUEVO REPORTE PDF ---
        ...
        if p_edit:
            ...
            if col_desc.button(
                "❌ Descartar selección y limpiar formulario",
                use_container_width=True,
            ):[cite: 1]
                st.session_state.proyecto_editar = {}[cite: 1]
                st.rerun()[cite: 1]
            ...
            if col_elim.button(
                "🗑️ Eliminar Proyecto Definitivamente",
                use_container_width=True,
            ):[cite: 1]
        ...
        with st.container(border=True):
            ...
            str_empresa = (
                cliente.strip() if cliente.strip() else "EMPRESA"
            )[cite: 1]
            ...
            responsables_guardados = p_edit.get("responsables", [])[cite: 1]
            if not isinstance(responsables_guardados, list):
                responsables_guardados = [
                    r.strip()
                    for r in str(responsables_guardados).split(",")
                    if r.strip()
                ][cite: 1]

            responsable_anterior = p_edit.get("responsable", "")[cite: 1]
            if responsable_anterior and not responsables_guardados:
                responsables_guardados = [
                    r.strip()
                    for r in str(responsable_anterior).split(",")
                    if r.strip()
                ][cite: 1]
            ...
            responsables_seleccionados = c7.multiselect(
                ...
                default=[
                    r
                    for r in responsables_guardados
            ...
            if nuevo_responsable.strip():
                nuevo_nombre = nuevo_responsable.strip()[cite: 1]
                if nuevo_nombre not in responsables_seleccionados:
                    responsables_seleccionados.append(nuevo_nombre)[cite: 1]
        ...
        with st.container(border=True):
            ...
            if "num_items" not in st.session_state:
                st.session_state.num_items = 2[cite: 1]
        ...
        with st.container(border=True):
            ...
            peso_corte_conf_auto = round(
                peso_total_recibido
                * st.session_state.pct_aprovechamiento_random,
                2,
            )[cite: 1]
            ...
            for i, item_fijo in enumerate(etapas_fijas):
                ...
                val_peso_etapa = float(item_fijo["peso_defecto"])[cite: 1]
        ...
        with st.container(border=True):
            ...
            if "num_prods" not in st.session_state:
                st.session_state.num_prods = 2[cite: 1]
            ...
            for i in range(st.session_state.num_prods):
                ...
                if prod_seleccionado == "➕ Otro (Escribir nuevo producto)":
                    ...
                    if (
                        nuevo_nombre.strip()
                        and nuevo_nombre.strip()
                        not in st.session_state.catalogo_productos
                    ):
                        st.session_state.catalogo_productos.insert(
                            -1, nuevo_nombre.strip()
                        )[cite: 1]
                else:
                    ...
                    nombre_final = prod_seleccionado[cite: 1]
        ...
        with st.container(border=True):
            ...
            pct_transf_auto = min(
                st.session_state.pct_transformado_ratio, pct_aprov_auto - 0.05
            )[cite: 1]
            pct_retazos_auto = pct_aprov_auto - pct_transf_auto[cite: 1]

            mat_transf_def = round(peso_total_recibido * pct_transf_auto, 2)[cite: 1]
            retazos_def = round(peso_total_recibido * pct_retazos_auto, 2)[cite: 1]
            perdida_def = (
                round(
            ...
            if peso_total_recibido > 0:
                pct_aprovechamiento_total = (
                    (mat_transformado + retazos_aprovechables)
                    / peso_total_recibido
                ) * 100[cite: 1]
                pct_perdida = (
                    perdida_no_aprovechable / peso_total_recibido
                ) * 100[cite: 1]
            else:
                pct_aprovechamiento_total = 0.0[cite: 1]
                pct_perdida = 0.0[cite: 1]

            st.markdown("##### Resumen de Indicadores")[cite: 1]
        ...
        with st.container(border=True):
            ...
            st.caption(
                "Destino Fijo: **Taller Las Flores, San Juan de Lurigancho"
                " (SJL)**"
            )[cite: 1]
            ...
            dist_defecto = float(DISTANCIAS_LIMA_SJL[distrito_sel])[cite: 1]
            ...
            st.caption(
                f"Distancia considerada: **{distancia_km:.1f} km**"
                f" ({recorrido_tipo}) | Emisión de Transporte estimada:"
            ...
            clav, ccort = st.columns(2)[cite: 1]
            clav.info(
                f"**Lavandería ({peso_lavado_auto:.2f} kg):**"
                f" {emisiones_lavado:.2f} kg CO₂e *(Factor: 0.30)*"
            )[cite: 1]
            ccort.info(
                f"**Corte ({peso_corte_auto:.2f} kg):**"
                f" {emisiones_corte:.2f} kg CO₂e *(Factor: 0.05)*"
            )[cite: 1]
            ...
            factor_bordado = FACTORES_BORDADO[tipo_diseno_bordado][cite: 1]
            emisiones_bordado = cant_prendas_bordado * factor_bordado[cite: 1]
        ...
        with st.container(border=True):
            ...
            if peso_total_recibido <= 10:
                dias_calc_corte, hdia_calc_corte = 1, 3.0[cite: 1]
                dias_calc_log, hdia_calc_log = 1, 2.0[cite: 1]
            elif peso_total_recibido <= 30:
                dias_calc_corte, hdia_calc_corte = 1, 6.0[cite: 1]
                dias_calc_log, hdia_calc_log = 1, 3.0[cite: 1]
            elif peso_total_recibido <= 50:
                dias_calc_corte, hdia_calc_corte = 2, 6.0[cite: 1]
                dias_calc_log, hdia_calc_log = 2, 3.0[cite: 1]
            else:
                dias_calc_corte = max(2, int(peso_total_recibido / 25))[cite: 1]
                hdia_calc_corte = 8.0[cite: 1]
                dias_calc_log = max(2, int(peso_total_recibido / 25))[cite: 1]
                hdia_calc_log = 4.0[cite: 1]
            ...
            h_col1.markdown("**Rol**")[cite: 1]
            h_col2.markdown("**Nombre**")[cite: 1]
            h_col3.markdown("**Editar**")[cite: 1]
            h_col4.markdown("**Días trabajados**")[cite: 1]
            h_col5.markdown("**Hora/día**")[cite: 1]
            h_col6.markdown("**Horas totales**")[cite: 1]
            ...
            for idx, p_fijo in enumerate(PERSONAL_FIJO_OPERACIONES):
                ...
                if rol_val == "Logística":
                    val_dias_defecto = dias_calc_log[cite: 1]
                    val_hdia_defecto = hdia_calc_log[cite: 1]
                else:
                    val_dias_defecto = dias_calc_corte[cite: 1]
                    val_hdia_defecto = hdia_calc_corte[cite: 1]
            ...
            with st.expander(
                "⚙️ Administrar Catálogo de Personal (Agregar, Modificar o"
                " Eliminar)"
            ):[cite: 1]
                ...
                with tab_add:
                    c_a1, c_a2 = st.columns([3, 1])[cite: 1]
                    ...
                    if c_a2.button(
                        "Guardar en Lista", use_container_width=True
                    ):[cite: 1]
                        n_limpio = nuevo_integrante.strip()[cite: 1]
                        if (
                            n_limpio
                            and n_limpio
                            not in st.session_state.lista_personal_confeccion
                        ):
                            st.session_state.lista_personal_confeccion.append(
                                n_limpio
                            )[cite: 1]
                            st.session_state.lista_personal_confeccion.sort()[cite: 1]
                            st.toast(f"✅ Agregado/a: {n_limpio}")[cite: 1]
                            st.rerun()[cite: 1]
                        elif (
                            n_limpio
                            in st.session_state.lista_personal_confeccion
                        ):
                            st.warning(
                                "⚠️ Esta persona ya se encuentra en la lista."
                            )[cite: 1]

                with tab_edit:
                    c_e1, c_e2, c_e3 = st.columns([2, 2, 1])[cite: 1]
                    ...
                    if c_e3.button("Actualizar", use_container_width=True):[cite: 1]
                        if (
                            nombre_modificado.strip()
                            and pers_a_mod
                            in st.session_state.lista_personal_confeccion
                        ):
                            idx_mod = (
                                st.session_state.lista_personal_confeccion.index(
                                    pers_a_mod
                                )
                            )[cite: 1]
                            st.session_state.lista_personal_confeccion[
                                idx_mod
                            ] = nombre_modificado.strip()[cite: 1]
                            st.session_state.lista_personal_confeccion.sort()[cite: 1]
                            st.toast(
                                f"✅ Actualizado: {nombre_modificado.strip()}"
                            )[cite: 1]
                            st.rerun()[cite: 1]

                with tab_del:
                    c_d1, c_d2 = st.columns([3, 1])[cite: 1]
                    pers_a_borrar = c_d1.selectbox(
                        "Persona a eliminar del catálogo:",
                        st.session_state.lista_personal_confeccion,
                        key="adm_sel_del",
                    )[cite: 1]
                    if c_d2.button("Eliminar", use_container_width=True):[cite: 1]
                        if (
                            pers_a_borrar
                            in st.session_state.lista_personal_confeccion
                        ):
                            st.session_state.lista_personal_confeccion.remove(
                                pers_a_borrar
                            )[cite: 1]
                            st.toast(f"🗑️ Eliminado/a: {pers_a_borrar}")[cite: 1]
                            st.rerun()[cite: 1]
            ...
            for idx, prod in enumerate(lista_productos):
                ...
                key_num_pers = f"num_pers_prod_{idx}"[cite: 1]
                if key_num_pers not in st.session_state:
                    st.session_state[key_num_pers] = 1[cite: 1]

                col_b1, col_b2, _ = st.columns([1.5, 1.5, 5])[cite: 1]
                if col_b1.button("➕  Persona", key=f"add_pers_{idx}"):[cite: 1]
                    st.session_state[key_num_pers] += 1[cite: 1]
                    st.rerun()[cite: 1]
                if (
                ...
                for p_idx in range(st.session_state[key_num_pers]):
                    ...
                    opciones_personas = list(
                        st.session_state.lista_personal_confeccion
                    )[cite: 1]
                    opcion_otro = "➕ Otro (Escribir nuevo nombre)"[cite: 1]
                    if opcion_otro not in opciones_personas:
                        opciones_personas.append(opcion_otro)[cite: 1]
                    ...
                    if persona_sel == opcion_otro:
                        ...
                        persona_nom = (
                            ...
                            if nuevo_nombre_escrito.strip()
                            else f"Persona {p_idx+1}"
                        )[cite: 1]
                        if (
                            nuevo_nombre_escrito.strip()
                            and nuevo_nombre_escrito.strip()
                            not in st.session_state.lista_personal_confeccion
                        ):
                            st.session_state.lista_personal_confeccion.append(
                                nuevo_nombre_escrito.strip()
                        ...
                        persona_nom = persona_sel[cite: 1]

                    cant_sugerida = (
                        max(1, int(p_cant / st.session_state[key_num_pers]))
                        if p_cant > 0
                        else 0
                    )[cite: 1]
                    ...
                    if rol_sel == "Acabado":
                        tiempo_unitario = round(tiempo_base_ia * 0.20, 3)[cite: 1]
                        c_tiempo.text_input(
                    ...
                    if persona_nom.strip():
                        personas_confeccion_set.add(persona_nom.strip())[cite: 1]
        ...
        with st.container(border=True):
            ...
            st.caption(
                "Agrega fotografías adicionales de colaboradoras con sus"
                " productos, procesos en taller, etc."
            )[cite: 1]
            ...
            if col_anx1.button("➕     Agregar Anexo"):[cite: 1]
                ...
                st.rerun()[cite: 1]
            if (
                col_anx2.button("➖     Quitar Anexo")
                and st.session_state.num_anexos > 0
            ):[cite: 1]
                st.session_state.num_anexos -= 1[cite: 1]
                st.rerun()[cite: 1]
            ...
            for a_i in range(st.session_state.num_anexos):
                ...
                if foto_anx is not None:
                    col_afoto.image(foto_anx, width=110)[cite: 1]
        ...
        def _validar_informe_final(
            cliente_val, ruc_val, responsable_val, origen_val, items_val
        ):
            errores = []
            if not cliente_val.strip():
                errores.append("Falta 'Cliente / Empresa' en Ficha General.")[cite: 1]
            if not ruc_val.strip() or not re.fullmatch(
                r"\d{11}", ruc_val.strip()
            ):
                errores.append("El 'RUC' debe tener 11 dígitos numéricos.")[cite: 1]
            if not responsable_val.strip():
                errores.append("Falta 'Responsable' en Ficha General.")[cite: 1]
            if not origen_val.strip():
                errores.append("Falta 'Punto Origen' en Ficha General.")[cite: 1]
            for i_item, v_item in enumerate(items_val, 1):
                if v_item["unidades"] <= 0 or v_item["peso_total"] <= 0:
                    errores.append(
                        f"En Ingreso de Material (Ítem {i_item}), las unidades y"
                        " el peso total deben ser mayores a 0."
                    )[cite: 1]
            return errores[cite: 1]
                    ...
                    with st.spinner("Guardando en la base de datos..."):
                        ...
                        if p_edit.get("id"):
                            supabase.table("proyectos").update(
                                datos_borrador
                            ).eq("id", p_edit["id"]).execute()[cite: 1]
                        else:
                            supabase.table("proyectos").insert(
                                datos_borrador
                            ).execute()[cite: 1]

                    st.success("✅ Borrador guardado exitosamente.")[cite: 1]
                    st.session_state.proyecto_editar = {}[cite: 1]
                    st.rerun()[cite: 1]

                except Exception as e:
                    st.error(f"⚠️ Error al guardar el borrador: {e}")[cite: 1]
                ...
                if errores_final:
                    ...
                    with st.spinner(
                        ...
                        try:
                            ...
                            pdf_bytes = pdf_buffer.getvalue()[cite: 1]
                            nombre_pdf_remoto = f"Informe_{codigo_proy}.pdf"[cite: 1]
                            ...
                            st.error(f"❌ Error al generar el PDF: {e}")[cite: 1]

            # --- BOTÓN 2: CONSTANCIA OFICIAL ---
            if col_gen2.button(
