...
def subir_pdf_supabase(nombre_archivo: str, pdf_bytes: bytes) -> str:
    ...
    try:
        supabase.storage.from("reportes").upload(
            path=nombre_archivo,
            file=pdf_bytes,
            file_options={"content-type": "application/pdf", "upsert": "true"},
        )
        url_res = supabase.storage.from("reportes").get_public_url(nombre_archivo)
        return url_res
    except Exception as e:
        st.caption(f"ℹ️ Nota de almacenamiento: {e}")
        return ""
...
def generar_pdf_oficial(
    ...
    elements.append(Paragraph("INFORME TÉCNICO DE VALORIZACIÓN TEXTIL", h1_style))[cite: 1]
    elements.append(
        Paragraph(
    ...
    elements.append(Paragraph("1. FICHA GENERAL DEL PROYECTO Y TRAZABILIDAD", h2_style))[cite: 1]
    elements.append(
        Paragraph(
    ...
    t_prendas = Table(data_prendas_pdf, colWidths=[30, 180, 80, 75, 75, 100])[cite: 1]
    t_prendas.setStyle(
        TableStyle([
    ...
    elements.append(Paragraph("3. TRAZABILIDAD DEL PROCESO EN UPCYCLING", h2_style))[cite: 1]
    elements.append(
        Paragraph(
    ...
    elements.append(Spacer(1, 6))[cite: 1]

    elements.append(Paragraph("<b>Operaciones – Corte y Logística</b>", cell_bold))[cite: 1]
    elements.append(
        Paragraph(
    ...
    # --- 9. SECCIÓN ANEXOS ---
    anexos_validos = [a for a in (lista_anexos or []) if a.get("foto") or a.get("nota", "").strip()][cite: 1]
    if anexos_validos:
        elements.append(PageBreak())[cite: 1]
        ...
        for idx_a, anexo in enumerate(anexos_validos, 1):
            ...
            nota_texto = anexo["nota"].strip() if anexo["nota"].strip() else "Sin descripción adicional."[cite: 1]
...
if "pct_aprovechamiento_random" not in st.session_state:
    st.session_state.pct_aprovechamiento_random = round(random.uniform(0.88, 0.94), 4)[cite: 1]

if "pct_transformado_ratio" not in st.session_state:
    st.session_state.pct_transformado_ratio = round(random.uniform(0.78, 0.83), 4)[cite: 1]
...
if not st.session_state.autenticado:
    ...
    if st.session_state.pestaña_activa == "⚡     Carga Rápida Histórica":
        ...
        if st.button("🚀 Guardar Proyecto Histórico Directamente", type="primary", use_container_width=True):[cite: 1]
            if not fast_cliente.strip():
                ...
                try:
                    with st.spinner("Registrando proyecto histórico..."):[cite: 1]
                    ...
                    st.success(f"✅ ¡Proyecto **{fast_cliente}** registrado exitosamente en el Histórico!")[cite: 1]
    ...
    # --- VISTA: PROYECTOS EN PROCESO ---
        ...
        st.caption("Proyectos guardados pendientes de culminación o emisión definitiva.")[cite: 1]
    ...
    # --- VISTA: DASHBOARD 2026 ---
        ...
        st.caption("Métricas consolidadas de todos los proyectos completados en el sistema.")[cite: 1]
        ...
        if completados:
            ...
            df_tabla = df_comp[[c for c in columnas_mostrar if c in df_comp.columns]].rename(columns=nombres_cols)[cite: 1]
            st.dataframe(df_tabla, use_container_width=True, hide_index=True)[cite: 1]
        else:
            st.info("📭 Aún no hay proyectos completados para mostrar en las métricas.")[cite: 1]
    ...
    # --- VISTA: HISTORIAL COMPLETO ---
        ...
        st.caption("Listado general de todos los proyectos registrados con acceso directo a sus Informes Técnicos PDF.")
        ...
        if proyectos_lista:
            for p in proyectos_lista:
                with st.container(border=True):
                    ...
                    # Botón de Descarga / Apertura de PDF si existe URL guardada
                    pdf_link = p.get("pdf_url")
                    if pdf_link:
                        hc4.link_button("📥 Ver / Descargar PDF", pdf_link, use_container_width=True)
                    else:
                        hc4.caption("📄 Sin PDF en nube")

                    if hc5.button("🗑️", key=f"hist_del_{p.get('id', p.get('codigo'))}", use_container_width=True, help="Eliminar proyecto"):
    ...
    # --- VISTA: NUEVO REPORTE PDF ---
        ...
        with st.container(border=True):
            ...
            origen = st.text_input("Punto Origen *", value=p_edit.get("origen", ""))[cite: 1]
        ...
        with st.container(border=True):
            ...
            for i, item_fijo in enumerate(etapas_fijas):
                ...
                permitir_editar = c_edit_chk.checkbox("✏️ Editar", key=f"chk_edit_{i}")[cite: 1]
            ...
            for i in range(st.session_state.num_prods):
                ...
                if prod_seleccionado == "➕ Otro (Escribir nuevo producto)":
                    ...
                    if (
                        ...
                        st.session_state.catalogo_productos.insert(-1, nuevo_nombre.strip())[cite: 1]
                else:
        ...
        with st.container(border=True):
            ...
            perdida_def = round(peso_total_recibido - mat_transf_def - retazos_def, 2) if peso_total_recibido > 0 else 0.0[cite: 1]

            editar_balance = st.checkbox("✏️ Editar balance manualmente", key="chk_edit_balance")[cite: 1]
        ...
        with st.container(border=True):
            ...
            st.caption("Destino Fijo: **Taller Las Flores, San Juan de Lurigancho (SJL)**")[cite: 1]
        ...
        with st.container(border=True):
            col_gen1, col_gen2, col_gen3 = st.columns([1, 1, 1])[cite: 1]

            if col_gen2.button(
                "💾 Guardar como Borrador", use_container_width=True
            ):[cite: 1]
                try:
                    with st.spinner("Guardando en la base de datos..."):[cite: 1]
                        datos_borrador = {
                            "codigo": codigo_proy,
                            "cliente": cliente,
                            "ruc": ruc,
            ...
            if col_gen1.button(
                ...
                if errores_final:
                    st.error("⚠️  Por favor, corrige los siguientes errores antes de generar el PDF:")[cite: 1]
                    for err in errores_final:
                        st.markdown(f"- {err}")[cite: 1]
                else:
                    with st.spinner("Generando y guardando Informe Técnico PDF en la nube..."):
                        try:
                            ...
                            url_pdf_subido = subir_pdf_supabase(nombre_pdf_remoto, pdf_bytes)

                            st.success("✅  ¡PDF generado y sincronizado con el Histórico!")
                            st.download_button(
                            ...
                            try:
                                datos_completado = {
                                    ...
                                    "productos_unids": total_prod_unid,
                                    "punto_origen": origen,
                                    "pdf_url": url_pdf_subido if url_pdf_subido else p_edit.get("pdf_url", ""),
                                }
                                if p_edit.get("id"):
                                ...
                                st.warning(f"⚠️ El PDF se generó, pero hubo un detalle al actualizar el estado en la BD: {e_bd}")
